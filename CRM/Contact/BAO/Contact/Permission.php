...
class CRM_Contact_BAO_Contact_Permission {
    ...
    foreach ($directions as $direction) {
      ...
      if ($direction['from'] == 'a') {
        $queries[] = "
          ...
          WHERE civicrm_relationship.{$user_id_column} IN ({$contact_id_list})
      ...
      foreach ($directions as $first_direction) {
        foreach ($directions as $second_direction) {
          ...
          $queries[] = "
            ...
            LEFT JOIN civicrm_relationship second_degree_relationship 
            ON first_degree_relationship.contact_id_{$first_direction['to']} = second_degree_relationship.contact_id_{$first_direction['from']}
            {$LEFT_JOIN_DELETED}
  ...
  public static function allowList($contact_ids, $type = CRM_Core_Permission::VIEW)
  {
    ...
    $result_set[(int)$contactID] = true;

    if (!CRM_Core_Permission::check('edit assigned contacts')) {
      return array_keys($result_set);
    }
  ...
  public static function allow($id, $type = CRM_Core_Permission::VIEW)
  {
    ...
    if ($type == CRM_Core_Permission::EDIT && !CRM_Core_Permission::check('edit assigned contacts')) {
      return false;
    }

    // check permission based on relationship, CRM-2963
    if (self::relationshipList([$id])) {
      return TRUE;
    }

    $permissionTable = _relatedpermissions_get_permissionedtable($contactID);

    $allowIds = [$contactID];

    $sql = 'SELECT contact_id FROM ' . $permissionTable;

    $dao = CRM_Core_DAO::executeQuery($sql);

    while ($dao->fetch()) {
      array_push($allowIds, $dao->contact_id);
    }

    if ($type == CRM_Core_Permission::EDIT) {
      $sql = '
        SELECT civicrm_relationship.contact_id_b AS contact_id
        FROM civicrm_relationship
        
        LEFT JOIN civicrm_contact ON civicrm_contact.id = contact_id_b 
        
        WHERE civicrm_relationship.contact_id_a IN (' . implode(',', $allowIds) . ')
        AND civicrm_relationship.contact_id_b = ' . $id . '
        AND civicrm_relationship.is_active = 1
        AND civicrm_relationship.is_permission_a_b = 1
        AND civicrm_contact.is_deleted = 0
      ';
    } else if ($type == CRM_Core_Permission::VIEW) {
      $sql = '
        (
          SELECT civicrm_relationship.contact_id_b AS contact_id 
          FROM civicrm_relationship
          ...
          AND civicrm_contact.is_deleted = 0
          AND (civicrm_relationship.contact_id_b = ' . $id . ' 
          AND civicrm_relationship.contact_id_a IN (' . implode(',', $allowIds) . ')
          AND (civicrm_relationship.is_permission_a_b = 1 OR civicrm_relationship.is_permission_a_b_v = 1)) 
        )
        UNION
        (
          SELECT civicrm_relationship.contact_id_a AS contact_id 
          FROM civicrm_relationship
          
          LEFT JOIN civicrm_contact ON civicrm_contact.id = contact_id_a
          
          WHERE civicrm_relationship.is_active = 1
          AND civicrm_contact.is_deleted = 0
          AND (civicrm_relationship.contact_id_a = ' . $id . ' 
          AND civicrm_relationship.contact_id_b IN (' . implode(',', $allowIds) . ')
          AND (civicrm_relationship.is_permission_b_a = 1 OR civicrm_relationship.is_permission_b_a_v = 1)) 
        )
      ';
    }

      ...
      <table class="crm-info-panel">
        {foreach from=$viewRelationship item="row"}
          ...
          <tr>
            ...
            <td>
              {if $row.is_permission_a_b_v or $row.is_permission_b_a_v}
                {if $row.is_permission_a_b_v}
                  <div>
                  {if $row.rtype EQ 'a_b' AND $is_contact_id_a}
  ...
  {if $action eq 2 or $action eq 1} {* add and update actions *}
    <div class="crm-block crm-form-block crm-relationship-form-block">
      <table class="form-layout-compressed">
        ...
        <tr class="crm-relationship-form-block-is_permission_a_b_v">
          {capture assign="contact_b"}{if $action eq 1}{ts}selected contact(s){/ts}{else}{$display_name_b}{/if}{/capture}
          <td class="label"></td>
          <td>
            {$form.is_permission_a_b_v.html}
            {ts 1=$display_name_a 2=$contact_b}<strong>%1</strong> can view information about %2.{/ts}
          </td>
        </tr>
        <tr class="crm-relationship-form-block-is_permission_b_a_v">
          <td class="label"></td>
          <td>
            {$form.is_permission_b_a_v.html}
            {ts 1=$contact_b|ucfirst 2=$display_name_a}<strong>%1</strong> can view information about %2.{/ts}
          </td>
        </tr>
